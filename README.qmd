...
- The density function of $X$ for each class $f_{k}$ follows a **Normal (Gaussian) distribution** within each class.Even though, it is often remarkably robust to model violations like Boolean variables.
- $X$ has a **common variance** across all $Y$ classes $\hat{\sigma}_{1}^2 = \dots = \hat{\sigma}_{k}^2$

To understand how the model calculate its parameters, let's see the **discriminant function** for one predictor and a response with 2 classes.
...
To represent the *discriminant function* in for many predictors we need to define the next elements:
...
\end{split}                      
$$
If we want to use the LDA for $K > 2$ we need to define the class $K$ as a baseline and the *discriminant function* would take the next form:
