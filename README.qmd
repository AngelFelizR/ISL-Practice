...
It models the **probability** ($p(X) = Pr(Y=1|X)$) that Y belongs to a particular category given some predictors. This model just support classifications between to categories and calculate the probability using the ***logistic function*** which produce a S form between 0 and 1:
...
As estimating the density function $f_{k}$ it's challenging models make some simplifying assumptions to estimate it.

## Linear Discriminant Analysis (LDA)

This model assumes that $f_{k}$ follow a **Normal (Gaussian) distribution** within each class, but it is often remarkably robust to model violations like Boolean variables.

If we have a problem, where $X = x$ and $\hat{\sigma}_{1}^2 = \dots = \hat{\sigma}_{k}^2$, we can estimate the mean $\hat{\mu}_{k}$ and the variance $\hat{\sigma}^2$ for $x$ and calculate $\hat{\pi}_{k} = n_{k}/ n$ for each $Y$ class. Then we can apply the $\log$ of $p_{k}(x)$ to represent the **discriminant function**:
...
In this functions, it's clear that a class $k$ has more possibilities to be selected as mean of $x$ for that particular class increases and its variance decreases. It is also important to take in consideration the effect of $\log{(\hat{\pi}_{k})}$, in consequence the proportion of classes also influence the results.
...
To represent the *discriminant function* in for many predicts we need to define the next elements:
...
- But, each class has its own covariance matrix, $X \sim N(\mu_{k}, \Sigma_{k})$
...
Consequently, LDA is a much less ﬂexible classiﬁer than QDA, and so has substantially lower variance.

## Naive Bayes

To estimate $f_{k}(X)$ this model assumes that *Within the kth class, the p predictors are independent* and as consequence:
