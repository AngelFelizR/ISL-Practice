...
After some manipulations we can solve an **odd** that can take any value between 0 and $\infty$, where low probabilities are close to 0 and higher ones to $\infty$, it becomes clear that the logistic regression model has a **logit** that it's a lineal combination of X.

$$
\begin{split}

\underbrace{\frac{p(X)}{1 - p(X)}}_\text{Odd} & =
e^{\beta_{0}+\beta_{1}X} \\

\underbrace{\log \left( \frac{p(X)}{1 - p(X)} \right)}
